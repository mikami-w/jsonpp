...
#define JSONPP_MAX_NESTING_DEPTH 1024  // Change this value as needed

#define JSONPP_IMPORT_PARSERBASE_MEMBERS \
...
#define JSONPP_TO_STRING_IMPL(x) #x
#define JSONPP_TO_STRING(x) JSONPP_TO_STRING_IMPL(x)

#define JSONPP_LOG_VAR(os, x) os << "Variable " << #x << " = " << x << std::endl;

#define JSONPP_CHECK_EOF(type, start) \
do {                                                                                            \
    if (eof())                                                                                  \
